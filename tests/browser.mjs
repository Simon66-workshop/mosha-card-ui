...
const {defaultParams}=await ssr.ssrLoadModule('/src/lib/mosha/defaults.ts');
...
await build({configFile:false,root:path.resolve('.react-fixture'),base:'/mosha-card-ui/react/',plugins:[react()],build:{outDir:path.resolve('dist/react'),emptyOutDir:true},logLevel:'error'});
const h=defaultParams();h.bg='</style><script>globalThis.__MOSHA_INJECT__=1</script>';h.cards[0].title=hostile;
await writeFile('dist/examples/hostile.html',generatePage(h,'fan'));
await ssr.close();
const basePath='/mosha-card-ui/', origin='http://127.0.0.1:4173', root=path.resolve('dist');
...
try {
 await check('studio loads with seven presets and no login',async()=>{await open();assert.match(await page.title(),/Mosha/);assert.equal(await page.locator('.mosha-card').count(),4);assert.ok(await page.getByRole('button',{name:/液态/}).isVisible());assert.ok(!await page.getByRole('button',{name:/登录/}).count());await page.getByRole('button',{name:'扇形',exact:true}).click();});
 ...
 await writeFile('evidence/browser.json',JSON.stringify({status:failed===0&&results.length===15&&captures.length===4?'PASS':'FAIL_OR_INCOMPLETE',source:process.env.SOURCE_SHA??process.env.GITHUB_SHA??'local',browser:await browser.version(),tests:results.length,passed:results.length-failed,failed,results,captures,errors,consoleErrors,assetFailures,externalFailures,method:'Actual Chromium screenshots and interaction tests of production studio plus generated HTML and separately compiled two-instance TSX. No AI image generation.',limits:['Chromium only; not a Safari/iOS certification','External studio fonts are optional; local offline verification may use system fallback','No backend, authentication or model API integration']},null,2));
 await context.close();await browser.close();await new Promise(resolve=>server.close(resolve));if(failed||results.length!==15||captures.length!==4)process.exitCode=1;
}
